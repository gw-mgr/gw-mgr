...
<script type="text/javascript">
	var tSXOrderManage;
	$(function() {
		tSXOrderManage = $('#tSXOrderManage')
				.datagrid(
						{
							url : '${path}/mgr/tOrderManage/orderList?orderType=ROOTRS',
							striped : true,
							rownumbers : true,
							pagination : true,
							singleSelect : false,
							...
							frozenColumns : [ [
									...
									},
									{
										...
										formatter : function(value, row, index) {
											...
											str += $
													.formatString(
															'<a href="javascript:void(0)" class="tSXOrderManage-easyui-linkbutton-look" data-options="plain:true,iconCls:\'fi-eye icon-blue\'" onclick="tOrderInfoLookFun(\'{0}\');" ></a>',
															row.id);
											</shiro:hasPermission>
											return str;
										}
							...
							onLoadSuccess : function(data) {
								$('.tSXOrderManage-easyui-linkbutton-look')
										.linkbutton({
											text : '查看'
										});
							}
			...
			if (b) {
				...
				$.post('${path}/mgr/tMerchantManage/delete', {
					...
					if (result.success) {
						...
						tSXOrderManage.datagrid('reload');
					}
					progressClose();
				}, 'JSON');
			}
		});
		tSXOrderManage.datagrid("unselectAll");
	}
	// 清除
	function tSXOrderManageCleanFun() {
		$('#tSXOrderManageSearchForm input').val('');
		tSXOrderManage.datagrid('load', {});
	}
	// 搜索
	function tSXOrderManageSearchFun() {
		tSXOrderManage.datagrid('load', $
				.serializeObject($('#tSXOrderManageSearchForm')));
	}
	// 导出EXCEL
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'north',border:false" style="height: 160px; overflow: hidden; background-color: #fff">
		...
		<form id="tSXOrderManageSearchForm">
			...
			<div style="width: 1200px; border: 0px solid;">
				<div style="border: 0px solid; float: left; width: 200px; height: 25px; font-size: 16px; padding-top: 1px;margin-left: 60px">
					关键字搜索&nbsp;&nbsp;
					<select name="keywordType" style="width: 80px; height: 25px; font-size: 13px">
						<option value="p.CAR_NUM">车牌号</option>
						<option value="p.CAR_OWNER">车主</option>
				...
				<div style="border: 0px solid; padding-top: 1px; height: 25px">
					...
					<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'fi-magnifying-glass',plain:true" onclick="tSXOrderManageSearchFun()">查询</a>
					<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'fi-x-circle',plain:true" onclick="tSXOrderManageCleanFun();">清空</a>
				</div>
			</div>
		</form>
		<br>
	...
	<div data-options="region:'center',border:false">
		<table id="tSXOrderManage" data-options="fit:true,border:false"></table>
	</div>
</div>
